...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        return [[
            [self.start_of_word] + [self.char2id.get(char, self.char_unk) for char in word] + [self.end_of_word] 
            for word in s] 
            for s in sents]
...
#         for x in char_ids: print(x, len(x))
        sents_padded = pad_sents_char(char_ids, self.char2id['<pad>'])
#         for x in sents_padded: print(x, len(x[0]))
