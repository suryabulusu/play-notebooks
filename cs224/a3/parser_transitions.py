...
def minibatch_parse(sentences, model, batch_size):
    ...
    unfin_parses = partial_parses[:] # same ref

    while unfin_parses != []:
      transitions = model.predict(unfin_parses[:batch_size])
      for i, transition in enumerate(transitions): unfin_parses[i].parse_step(transition)
      unfin_parses = [elem for elem in unfin_parses[:batch_size] if len(elem.stack) != 1 or elem.buffer != []] \
                      + unfin_parses[batch_size: ]
