...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2, no_char_decoder=False):
        ...
        self.attn_projection = nn.Linear(2 * self.hidden_size, self.hidden_size, bias = False)
...
#         print(source_padded.shape)
        ...
        enc_hiddens_unpacked, lens_unpacked = pad_packed_sequence(enc_hiddens, batch_first = True) # (B, T, 2*h)
        
        last_hidden = torch.cat((last_hidden[1], last_hidden[0]), dim = 1)
        last_cell = torch.cat((last_cell[1], last_cell[0]), dim = 1)
        
        init_decoder_hidden = self.h_projection(last_hidden)
        init_decoder_cell = self.c_projection(last_cell)
        
        return enc_hiddens_unpacked, (init_decoder_hidden, init_decoder_cell)
...
#         return enc_hiddens, dec_init_state
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        enc_hiddens_proj = self.attn_projection(enc_hiddens)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        ### COPY OVER YOUR CODE FROM ASSIGNMENT 4
        dec_state = self.decoder(Ybar_t, dec_state) # input, h_0, c_0
        dec_hidden, dec_cell = dec_state
        
        e_t = torch.einsum("abc, ac -> ab", enc_hiddens_proj, dec_hidden)
        ...
        a_t = torch.einsum("ab, abc -> ac", alpha_t, enc_hiddens)
    ...
    def beam_search(self, src_sent: List[str], beam_size: int=5, max_decoding_time_step: int=70) -> List[Hypothesis]:
        ...
        src_sents_var = self.vocab.src.to_input_tensor_char([src_sent], device = self.device)

        src_encodings, dec_init_vec = self.encode(src_sents_var, [len(src_sent)])
        src_encodings_att_linear = self.attn_projection(src_encodings)
...
#             print(t)
            ...
            ## End A4 code
#             print(list([hyp[-1]] for hyp in hypotheses))
    ...
    def device(self) -> torch.device:
        ...
        """
#         print("I am defined doston..")
        return self.attn_projection.weight.device
    ...
    def save(self, path: str):
        ...
        params = {
            'args': dict(embed_size=self.model_embeddings_source.word_emb_size, hidden_size=self.hidden_size, dropout_rate=self.dropout_rate),
            'vocab': self.vocab,
